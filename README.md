...
Ans - No. assignment to expression with array type - a = (int []) {4, 5, 6}; Error note seems to be pretty explanatory. However similar syntax is used for structured objects.
...
Can we modify a value given that, the correpsonding location is pointed by a variable which is wrapped in a struct and struct's type is prefixed with const?
...
Ans - https://www.eskimo.com/~scs/cclass/int/sx9a.html

-----

Can a volatile member be const; so that software ensures read-only operations?

And - Yes. This can be handy typically in embedded codes where hardware might control value and software mainly listens the value. Or, may be compiler optimizes consts as they are constants thorugh out the code and so this trick might help to (kind of) disable optimizations involving the const variable (from math point of view, constant and variable are opposite - C fused opposite words).
...
Ans - (typecheck.h in Linux). Usually used in macro body. As we wouldn't know type of argument that is expanded.
...
Though unions save memory, we can't use it's members all at once; So, why did designers add it in C grammer? What's the use of unions?

Ans - One feature is aliasing - you can give more than one name to a memory location. Assume that, an union contains an integer; And also the same union hosts an array of bits. With this setup, we can set or clear all the bits of a memory location and it may be useful in the embedded world. The above expample can pullup all the bits of a port OR pull-down all the bits with just one assignment statement involving the integer member. Another example the I found on Stackoverflow community is - float, doubles are IEEE754 complaint (most-ly). So, bits in each field of float can extracted using unions in easy manner without using deep bit manipulation operators.
